...
def Cext_FIT(holo, pixel_size, z, fuoco, lim, k, x_fit_1, media, name_graph_3d, name_graph_2d):
    ...
    try:
        ...
        im2 = ax2.imshow(data_fitted.reshape(int(lim*2), int(lim*2)), cmap='viridis', extent=[
   ...
   # x1=np.arange(0,numero_linea,1)
    ...
    if len(inviluppo_sup) > 0 and len(inviluppo_min) > 0:
        if len(inviluppo_sup) > len(inviluppo_min):
            ...
            y = ((Integrale_array[inviluppo_sup[0:len(inviluppo_min)]] -
                  Integrale_array[inviluppo_min])/2+Integrale_array[inviluppo_min])
        if len(inviluppo_sup) < len(inviluppo_min):
            x2 = inviluppo_sup
            y = ((Integrale_array[inviluppo_sup]-Integrale_array[inviluppo_min[0:len(
                inviluppo_sup)]])/2+Integrale_array[inviluppo_min[0:len(inviluppo_sup)]])
        if len(inviluppo_sup) == len(inviluppo_min):
            x2 = inviluppo_sup
        ...
        y2 = np.ones(len(x3))*((Integrale_array[inviluppo_sup[0]] -
                                Integrale_array[inviluppo_min[0]])/2+Integrale_array[inviluppo_min[0]])
